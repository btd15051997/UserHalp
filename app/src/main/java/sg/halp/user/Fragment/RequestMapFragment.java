...
import sg.halp.user.Adapter.AmbulanceOperatorVerticalAdaper;
import sg.halp.user.Adapter.PaymentModeAdapter;
import sg.halp.user.Adapter.SimpleSpinnerAdapter;
import sg.halp.user.Dialog.HospitalDischargeWarningDialog;
...
public class RequestMapFragment extends BaseFragment implements LocationHelper.OnLocationReceived, AsyncTaskCompleteListener, OnMapReadyCallback, AdapterCallback{
    ...
    private String TAG = RequestMapFragment.class.getSimpleName();
...
//    private int assistiveDeviceValue = 1;
    ...
    private ProgressBar ambulance_operator_loading;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ambulance_operator_loading = (ProgressBar) view.findViewById(R.id.progress_circular);
...
//        getTypesforhome();
    ...
    private void getAmbulanceOperators(String dis, String dur) {
        ...
        ambulance_operator_loading.setIndeterminate(true);
        ambulance_operator_loading.setVisibility(View.VISIBLE);
    ...
    public void drawPath(String result) {
        ...
        try {
            ...
            PolylineOptions options = new PolylineOptions().width(8).color(getResources().getColor(R.color.deeporange600)).geodesic(true);
...
/*
            ...
            case R.id.btn_request_cab:
                ...
                mRequestOptional = getTripOptional();
    ...
    private RequestOptional getTripOptional() {

        RequestOptional requestOptional = new RequestOptional();

        requestOptional = new RequestOptional();

        requestOptional.setId(new PreferenceHelper(activity).getUserId());
        requestOptional.setToken(new PreferenceHelper(activity).getSessionToken());

        if (pic_latlan != null) {

            requestOptional.setPic_lat(pic_latlan.latitude);
            requestOptional.setPic_lng(pic_latlan.longitude);
            EbizworldUtils.appLogDebug("HaoLS", "Pickup latitude: " + pic_latlan.latitude + "- longitude: " + pic_latlan.longitude);
        }
        if (drop_latlan != null) {

            requestOptional.setDrop_lng(drop_latlan.longitude);
            requestOptional.setDrop_lat(drop_latlan.latitude);

            EbizworldUtils.appLogDebug("HaoLS", "Drop latitude: " + drop_latlan.latitude + "- longitude: " + drop_latlan.longitude);

        }
        if (stop_latlan != null) {

            requestOptional.setIsAddStop(1);
            requestOptional.setAddStop_lng(stop_latlan.longitude);
            requestOptional.setAddStop_lat(stop_latlan.latitude);
            requestOptional.setAddStop_address(stop_address);
        }

        requestOptional.setOperator_id(Integer.parseInt(new PreferenceHelper(activity).getRequestType()));
        requestOptional.setPic_address(s_address);
        requestOptional.setDrop_address(d_address);
        requestOptional.setRequest_status_type(1);
        requestOptional.setPromoCode(promoCode);
        requestOptional.setRemark(trip_remark.getText().toString());

        if (cb_a_and_e_value.isChecked()){

            requestOptional.setA_and_e(1);

        }else {

            requestOptional.setA_and_e(0);
        }

        if (cb_imh_value.isChecked()){

            requestOptional.setImh(1);

        }else {

            requestOptional.setImh(0);
        }

        if (cb_ferry_terminals_value.isChecked()){

            requestOptional.setFerry_terminals(1);

        }else {

            requestOptional.setFerry_terminals(0);
        }

        if (cb_staircase_value.isChecked()){

            requestOptional.setStaircase(1);

        }else {

            requestOptional.setStaircase(0);

        }

        if (cb_tarmac_value.isChecked()){

            requestOptional.setTarmac(1);

        }else {

            requestOptional.setTarmac(0);

        }

        requestOptional.setFamily_member(familyMember);
        requestOptional.setHouseUnit(edt_house_unit.getText().toString().trim());
        requestOptional.setWeight(weight);
        requestOptional.setOxygen(oxygenTank);
        requestOptional.setCaseType(case_type);
    ...
    public void onTaskCompleted(final String response, int serviceCode) {
        ...
        switch (serviceCode) {
            ...
            case Const.ServiceCode.AMBULANCE_OPERATOR:
                ...
                ambulance_operator_loading.setIndeterminate(false);
                ambulance_operator_loading.setVisibility(View.GONE);
    ...
    private void showpaymentoptionlst(final ArrayList<Payments> paymentlst) {
        ...
        if (new PreferenceHelper(activity).getPaymentMode().equals("cod")) {
            ...
            tv_payment_title.setText(getResources().getString(R.string.txt_selected_type) + " " + getResources().getString(R.string.pre_select_payment_paypal));
        ...
        lv_cards.addOnItemTouchListener(new RecyclerLongPressClickListener(activity, lv_cards, new RecyclerLongPressClickListener.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (new PreferenceHelper(activity).getPaymentMode().equals("cod")) {
                    ...
                    tv_cashtype.setText(getResources().getString(R.string.txt_selected_type) + " " + getResources().getString(R.string.pre_select_payment_paypal));
                } else if (new PreferenceHelper(activity).getPaymentMode().equals("walletbay")) {
